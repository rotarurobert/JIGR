var paper = new function () {
	var Base = new function () {
		...
		function inject(dest, src, enumerable, base, preserve, generics) {
			...
			function field(name, val, dontCheck, generics) {
				...
				if ((dontCheck || val !== undefined && has.call(src, name)) && (!preserve || !prev)) {
					if (func) {
						if (prev && /\bthis\.base\b/.test(val)) {
							...
							res = function () {
								...
								try {
									...
									tmp ? define(this, 'base', tmp) : delete this.base;
								}
							};
						...
						if (beans && val.length == 0 && (bean = name.match(/^(get|is)(([A-Z])(.*))$/))) beans.push([bean[3].toLowerCase() + bean[4], bean[2]]);
					}
					if (!res || func || !res.get) res = {
						value: res
						, writable: true
					};
			...
			if (src) {
				beans = [];
				for (var name in src)
					if (has.call(src, name) && !hidden.test(name)) field(name, null, true, generics);
				field('toString');
				field('valueOf');
				for (var i = 0, l = beans && beans.length; i < l; i++) try {
					var bean = beans[i]
						, part = bean[1];
					field(bean[0], {
		...
		function extend(obj) {
			var ctor = function (dont) {
				...
				if (this.initialize && dont !== ctor.dont) return this.initialize.apply(this, arguments);
			}
			ctor.prototype = obj;
		...
		function iterator(iter) {
			return !iter ? function (val) {
				return val
			} : typeof iter !== 'function' ? function (val) {
				return val == iter
			} : iter;
		}
		...
		function each(obj, iter, bind, asArray) {
			try {
				if (obj)
					(asArray || asArray === undefined && isArray(obj) ? forEach : forIn).call(obj, iterator(iter), bind = bind || obj);
			}
			catch (e) {
		...
		return inject(function () {}, {
			inject: function (src) {
				if (src) {
					...
					if (statics != src) inject(proto, src, src.enumerable, base && base.prototype, src.preserve, src.generics && this);
			...
			, each: each
			, inject: function () {
				for (var i = 0, l = arguments.length; i < l; i++) inject(this, arguments[i]);
				return this;
			}
			, extend: function () {
		...
		, toString: function () {
			return '{ ' + Base.each(this, function (value, key) {
				if (key.charAt(0) != '_') {
					...
					this.push(key + ': ' + (type === 'number' ? Base.formatNumber(value) : type === 'string' ? "'" + value + "'" : value));
				}
			}, []).join(', ') + ' }';
		}
		, statics: {
			read: function (list, start, length) {
				...
				var obj = list[start];
				if (obj instanceof this || this.prototype._readNull && obj == null && length <= 1) return obj;
				obj = new this(this.dont);
				return obj.initialize.apply(obj, start > 0 || length < list.length ? Array.prototype.slice.call(list, start, start + length) : list) || obj;
			}
			, readAll: function (list, start) {
				var res = []
					, entry;
				for (var i = start || 0, l = list.length; i < l; i++) {
					res.push(Array.isArray(entry = list[i]) ? this.read(entry, 0) : this.read(list, i, 1));
				}
				return res;
			}
			, splice: function (list, items, index, remove) {
				...
				if (append) {
					...
					for (var i = 0, l = removed.length; i < l; i++) delete removed[i]._index;
			...
			, hyphenate: function (str) {
				return str.replace(/[a-z][A-Z0-9]|[0-9][a-zA-Z]|[A-Z]{2}[a-z]/g, function (match) {
	...
	var PaperScope = this.PaperScope = Base.extend({
		initialize: function (script) {
			...
			this.projects = [];
			this.tools = [];
			this._id = script && (script.getAttribute('id') || script.src) || ('paperscope-' + (PaperScope._id++));
			if (script) script.setAttribute('id', this._id);
		...
		, getTool: function () {
			if (!this._tool) this._tool = new Tool();
			return this._tool;
		}
		...
		, install: function (scope) {
			...
			for (var key in this) {
				if (!/^(version|_id|load)/.test(key) && !(key in scope)) scope[key] = this[key];
			}
		}
		...
		, statics: {
			...
			, _id: 0
			, get: function (id) {
				if (typeof id === 'object') id = id.getAttribute('id');
	...
	var Callback = {
		...
		, detach: function (type, func) {
			...
			if (entry && handlers) {
				if (!func || (index = handlers.indexOf(func)) != -1 && handlers.length == 1) {
					if (entry.uninstall) entry.uninstall.call(this, type);
		...
		, fire: function (type, event) {
			...
			Base.each(handlers, function (func) {
				if (func.call(this, event) === false && event && event.stop) event.stop();
			}, this);
			return true;
		}
	...
	var Point = this.Point = Base.extend({
		...
		, rotate: function (angle, center) {
			...
			var point = center ? this.subtract(center) : this
				, s = Math.sin(angle)
				, c = Math.cos(angle);
			point = Point.create(point.x * c - point.y * s, point.y * c + point.x * s);
		...
		, project: function (point) {
			...
			if (point.isZero()) {
				...
				return Point.create(point.x * scale, point.y * scale);
			}
		}
		, statics: {
			create: function (x, y) {
			...
			, min: function (point1, point2) {
				...
				return Point.create(Math.min(point1.x, point2.x), Math.min(point1.y, point2.y));
			}
			, max: function (point1, point2) {
				point1 = Point.read(arguments, 0, 1);
				point2 = Point.read(arguments, 1, 1);
				return Point.create(Math.max(point1.x, point2.x), Math.max(point1.y, point2.y));
			}
			, random: function () {
	...
	var LinkedPoint = Point.extend({
		set: function (x, y, dontNotify) {
			this._x = x;
			this._y = y;
			if (!dontNotify) this._owner[this._setter](this);
			return this;
		}
		, getX: function () {
			return this._x;
		...
		, statics: {
			create: function (owner, setter, x, y, dontLink) {
				if (dontLink) return Point.create(x, y);
	...
	var Size = this.Size = Base.extend({
		...
		, toString: function () {
			var format = Base.formatNumber;
			return '{ width: ' + format(this.width) + ', height: ' + format(this.height) + ' }';
		}
		...
		, statics: {
			...
			, min: function (size1, size2) {
				return Size.create(Math.min(size1.width, size2.width), Math.min(size1.height, size2.height));
			}
			, max: function (size1, size2) {
				return Size.create(Math.max(size1.width, size2.width), Math.max(size1.height, size2.height));
			}
			, random: function () {
	...
	var LinkedSize = Size.extend({
		...
		, statics: {
			create: function (owner, setter, width, height, dontLink) {
				if (dontLink) return Size.create(width, height);
	...
	var Rectangle = this.Rectangle = Base.extend({
		...
		, toString: function () {
			var format = Base.formatNumber;
			return '{ x: ' + format(this.x) + ', y: ' + format(this.y) + ', width: ' + format(this.width) + ', height: ' + format(this.height) + ' }';
		}
		...
		, expand: function (hor, ver) {
			if (ver === undefined) ver = hor;
			return Rectangle.create(this.x - hor / 2, this.y - ver / 2, this.width + hor, this.height + ver);
		}
		...
		return Base.each([
			...
			var x = parts[xFirst ? 0 : 1]
				...
				, set = 'set' + part;
			this[get] = function () {
				return LinkedPoint.create(this, set, this[getX](), this[getY](), arguments[0]);
			};
	...
	var LinkedRectangle = Rectangle.extend({
		...
		, statics: {
			create: function (owner, setter, x, y, width, height) {
				var rect = new LinkedRectangle(LinkedRectangle.dont).set(x, y, width, height, true);
				rect._owner = owner;
				rect._setter = setter;
				return rect;
			}
		...
		return Base.each(['x', 'y', 'width', 'height'], function (key) {
			...
			, 'LeftCenter', 'TopCenter', 'RightCenter', 'BottomCenter'], function (key) {
			var name = 'set' + key;
	...
	var Matrix = this.Matrix = Base.extend({
		...
		, toString: function () {
			...
			return '[[' + [format(this._a), format(this._b)
					, format(this._tx)].join(', ') + '], [' + [format(this._c), format(this._d)
		...
		, transform: function (src, srcOff, dst, dstOff, numPts) {
			return arguments.length < 5 ? this._transformPoint(Point.read(arguments)) : this._transformCoordinates(src, srcOff, dst, dstOff, numPts);
		}
		, _transformPoint: function (point, dest, dontNotify) {
			var x = point.x
				, y = point.y;
			if (!dest) dest = new Point(Point.dont);
			return dest.set(x * this._a + y * this._b + this._tx, x * this._c + y * this._d + this._ty, dontNotify);
		}
		...
		, _transformBounds: function (bounds, dest, dontNotify) {
			...
			if (!dest) dest = new Rectangle(Rectangle.dont);
		...
		, _getDeterminant: function () {
			...
			return isFinite(det) && Math.abs(det) > Numerical.EPSILON && isFinite(this._tx) && isFinite(this._ty) ? det : null;
		}
		, _inverseTransform: function (point, dest, dontNotify) {
			...
			if (!dest) dest = new Point(Point.dont);
			return dest.set(
				(x * this._d - y * this._b) / det, (y * this._a - x * this._c) / det, dontNotify);
		}
		...
		, getRotation: function () {
			...
			return Math.abs(angle1 - angle2) < Numerical.TOLERANCE ? angle1 * 180 / Math.PI : undefined;
		}
		, equals: function (mx) {
			return this._a == mx._a && this._b == mx._b && this._c == mx._c && this._d == mx._d && this._tx == mx._tx && this._ty == mx._ty;
		}
		, isIdentity: function () {
		...
		, createInverse: function () {
			var det = this._getDeterminant();
			return det && Matrix.create(this._d / det, -this._c / det, -this._b / det, this._a / det, (this._b * this._ty - this._d * this._tx) / det, (this._c * this._tx - this._a * this._ty) / det);
		}
		...
		, setToRotation: function (angle, center) {
			...
			var x = center.x
				...
				, cos = Math.cos(angle)
				, sin = Math.sin(angle);
			return this.set(cos, sin, -sin, cos, x - x * cos + y * sin, y - x * sin - y * cos);
		}
		, applyToContext: function (ctx, reset) {
			ctx[reset ? 'setTransform' : 'transform'](this._a, this._c, this._b, this._d, this._tx, this._ty);
			return this;
		}
		, statics: {
	...
	var Line = this.Line = Base.extend({
		...
		, intersect: function (line) {
			...
			if (Math.abs(cross) <= Numerical.EPSILON) return null;
			var v = line.point.subtract(this.point)
				, t1 = v.cross(line.vector) / cross
				, t2 = v.cross(this.vector) / cross;
			return (this.infinite || 0 <= t1 && t1 <= 1) && (line.infinite || 0 <= t2 && t2 <= 1) ? this.point.add(this.vector.multiply(t1)) : null;
		}
		...
		, getDistance: function (point) {
			...
			return this.infinite ? dist : Math.min(dist, point.getDistance(this.point), point.getDistance(this.point.add(this.vector)));
		}
	});
	var Project = this.Project = PaperScopeItem.extend({
		...
		, initialize: function (view) {
			...
			if (view) this.view = view instanceof View ? view : View.create(view);
		}
		, _needsRedraw: function () {
			if (this.view) this.view._redrawNeeded = true;
		}
		, remove: function () {
		...
		, selectAll: function () {
			for (var i = 0, l = this.layers.length; i < l; i++) this.layers[i].setSelected(true);
		}
		, deselectAll: function () {
			for (var i in this._selectedItems) this._selectedItems[i].setSelected(false);
		}
		...
		, draw: function (ctx, matrix) {
			ctx.save();
			if (!matrix.isIdentity()) matrix.applyToContext(ctx);
			var param = {
				offset: new Point(0, 0)
			};
			for (var i = 0, l = this.layers.length; i < l; i++) Item.draw(this.layers[i], ctx, param);
			ctx.restore();
			if (this._selectedItemCount > 0) {
				...
				function getGlobalMatrix(item, mx, cached) {
					...
					if (item._parent) {
						...
						if (!item._matrix.isIdentity()) mx.concatenate(item._matrix);
					}
					else {
						mx.initialize(item._matrix);
					}
					if (cached) matrices[item._id] = mx.clone();
					return mx;
				}
	...
	var Item = this.Item = Base.extend(Callback, {
		_events: new function () {
			...
			function onFrame(event) {
				for (var i = 0, l = onFrameItems.length; i < l; i++) onFrameItems[i].fire('frame', event);
			}
			return Base.each(['onMouseDown', 'onMouseUp', 'onMouseDrag', 'onClick'
			, 'onDoubleClick', 'onMouseMove', 'onMouseEnter', 'onMouseLeave'], function (name) {
				this[name] = mouseEvent;
			}, {
				onFrame: {
					install: function () {
						if (!onFrameItems.length) this._project.view.attach('frame', onFrame);
						onFrameItems.push(this);
					}
					, uninstall: function () {
						onFrameItems.splice(onFrameItems.indexOf(this), 1);
						if (!onFrameItems.length) this._project.view.detach('frame', onFrame);
					}
				}
			});
		}
		, initialize: function (pointOrMatrix) {
			...
			if (!this._style) this._style = PathStyle.create(this);
			this.setStyle(this._project.getCurrentStyle());
			this._matrix = pointOrMatrix !== undefined ? pointOrMatrix instanceof Matrix ? pointOrMatrix.clone() : new Matrix().translate(Point.read(arguments, 0)) : new Matrix();
		}
		, _changed: function (flags) {
			...
			if (this._parent && (flags & (ChangeFlag.GEOMETRY | ChangeFlag.STROKE))) {
			...
			if (this._parentSymbol) this._parentSymbol._changed(flags);
		...
		, isSelected: function () {
			if (this._children) {
				for (var i = 0, l = this._children.length; i < l; i++)
					if (this._children[i].isSelected()) return true;
			}
			return this._selected;
		}
		, setSelected: function (selected) {
			if (this._children && !arguments[1]) {
				for (var i = 0, l = this._children.length; i < l; i++) this._children[i].setSelected(selected);
			}
		...
		, isFullySelected: function () {
			if (this._children && this._selected) {
				for (var i = 0, l = this._children.length; i < l; i++)
					if (!this._children[i].isFullySelected()) return false;
				return true;
			}
			return this._selected;
		}
		, setFullySelected: function (selected) {
			if (this._children) {
				for (var i = 0, l = this._children.length; i < l; i++) this._children[i].setFullySelected(selected);
			}
		...
		, getPosition: function () {
			var pos = this._position || (this._position = this.getBounds().getCenter(true));
		...
		this['get' + Base.capitalize(name)] = function () {
			var type = this._boundsType
				, bounds = this._getCachedBounds(typeof type == 'string' ? type : type && type[name] || name, arguments[0]);
			return name == 'bounds' ? LinkedRectangle.create(this, 'setBounds', bounds.x, bounds.y, bounds.width, bounds.height) : bounds;
		};
	}, {
		_getCachedBounds: function (type, matrix, cacheItem) {
			...
			if (cacheItem && this._parent) {
				var id = cacheItem._id
					, ref = this._parent._boundsCache = this._parent._boundsCache || {
						ids: {}
						, list: []
					};
				if (!ref.ids[id]) {
			...
			if (cache && this._bounds && this._bounds[cache]) return this._bounds[cache];
			var identity = this._matrix.isIdentity();
			matrix = !matrix || matrix.isIdentity() ? identity ? null : this._matrix : identity ? matrix : matrix.clone().concatenate(this._matrix);
		...
		, _clearBoundsCache: function () {
			if (this._boundsCache) {
				for (var i = 0, list = this._boundsCache.list, l = list.length; i < l; i++) {
					var item = list[i];
					delete item._bounds;
					if (item != this && item._boundsCache) item._clearBoundsCache();
				}
		...
		, _clone: function (copy) {
			...
			if (this._children) {
				for (var i = 0, l = this._children.length; i < l; i++) copy.addChild(this._children[i].clone());
			}
			...
			if (this._name) copy.setName(this._name);
			return copy;
		}
		...
		, hitTest: function (point, options) {
			...
			if (!this._children && !this.getRoughBounds().expand(options.tolerance)._containsPoint(point)) return null;
			if ((options.center || options.bounds) && !(this instanceof Layer && !this._parent)) {
				var bounds = this.getBounds()
					, that = this
				...
				function checkBounds(type, part) {
					...
					if (point.getDistance(pt) < options.tolerance) return new HitResult(type, that, {
						name: Base.hyphenate(part)
						, point: pt
					});
				}
				if (options.center && (res = checkBounds('center', 'Center'))) return res;
				if (options.bounds) {
					for (var i = 0; i < 8; i++)
						if (res = checkBounds('bounds', points[i])) return res;
				}
			}
			return this._children || !(options.guides && !this._guide || options.selected && !this._selected) ? this._hitTest(point, options) : null;
		}
		...
		, addChildren: function (items) {
			for (var i = 0, l = items && items.length; i < l; i++) this.insertChild(undefined, items[i]);
		}
		, insertChildren: function (index, items) {
			for (var i = 0, l = items && items.length; i < l; i++) {
				if (this.insertChild(index, items[i])) index++;
			}
		}
		, insertAbove: function (item) {
			var index = item._index;
			if (item._parent == this._parent && index < this._index) index++;
		...
		, _remove: function (deselect, notify) {
			if (this._parent) {
				if (deselect) this.setSelected(false);
				if (this._name) this._removeFromNamed();
				if (this._index != null) Base.splice(this._parent._children, null, this._index, 1);
				if (notify) this._parent._changed(Change.HIERARCHY);
				this._parent = null;
				return true;
			}
			return false;
		...
		, removeChildren: function (from, to) {
			...
			if (removed.length > 0) this._changed(Change.HIERARCHY);
			return removed;
		}
		...
		, isGroupedWith: function (item) {
			...
			while (parent) {
				if (parent._parent && (parent instanceof Group || parent instanceof CompoundPath) && item.isDescendant(parent)) return true;
		...
		, scale: function (hor, ver, center, apply) {
			...
			return this.transform(new Matrix().scale(hor, ver, center || this.getPosition(true)), apply);
		}
		...
		, rotate: function (angle, center, apply) {
			return this.transform(new Matrix().rotate(angle, center || this.getPosition(true)), apply);
		}
		, shear: function (hor, ver, center, apply) {
			...
			return this.transform(new Matrix().shear(hor, ver, center || this.getPosition(true)), apply);
		}
		, transform: function (matrix, apply) {
			var bounds = this._bounds
				, position = this._position;
			this._matrix.preConcatenate(matrix);
			if (this._transform) this._transform(matrix);
			if (apply) this.apply();
			this._changed(Change.GEOMETRY);
		...
		, fitBounds: function (rectangle, fill) {
			...
			var bounds = this.getBounds()
				...
				, newBounds = new Rectangle(new Point(), Size.create(bounds.width * scale, bounds.height * scale));
		...
		, _setStyles: function (ctx) {
			...
			if (!fillColor || !strokeColor) ctx.globalAlpha = this._opacity;
		}
		, statics: {
			...
			, draw: function (item, ctx, param) {
				if (!item._visible || item._opacity == 0) return;
				var tempCanvas, parentCtx
					, itemOffset, prevOffset;
				if (item._blendMode !== 'normal' || item._opacity < 1 && !(item._segments && (!item.getFillColor() || !item.getStrokeColor()))) {
					var bounds = item.getStrokeBounds();
					if (!bounds.width || !bounds.height) return;
					prevOffset = param.offset;
					parentCtx = ctx;
					itemOffset = param.offset = bounds.getTopLeft().floor();
					tempCanvas = CanvasProvider.getCanvas(bounds.getSize().ceil().add(Size.create(1, 1)));
					ctx = tempCanvas.getContext('2d');
				}
				if (!param.clipping) ctx.save();
				if (tempCanvas) ctx.translate(-itemOffset.x, -itemOffset.y);
				item._matrix.applyToContext(ctx);
				item.draw(ctx, param);
				if (!param.clipping) ctx.restore();
				if (tempCanvas) {
					param.offset = prevOffset;
					if (item._blendMode !== 'normal') {
						BlendMode.process(item._blendMode, ctx, parentCtx, item._opacity, itemOffset.subtract(prevOffset));
					}
					else {
						parentCtx.save();
	...
	var Group = this.Group = Item.extend({
		initialize: function (items) {
			...
			this._children = [];
			this._namedChildren = {};
			this.addChildren(!items || !Array.isArray(items) || typeof items[0] !== 'object' ? arguments : items);
		}
		, _changed: function (flags) {
		...
		, _getClipItem: function () {
			...
			for (var i = 0, l = this._children.length; i < l; i++) {
				var child = this._children[i];
				if (child._clipMask) return this._clipItem = child;
			}
		...
		, draw: function (ctx, param) {
			...
			for (var i = 0, l = this._children.length; i < l; i++) {
				...
				if (item != clipItem) Item.draw(item, ctx, param);
			}
		}
	});
	var Layer = this.Layer = Group.extend({
		...
		, _remove: function (deselect, notify) {
			if (this._parent) return this.base(deselect, notify);
			if (this._index != null) {
				if (deselect) this.setSelected(false);
		...
		function insert(above) {
			return function (item) {
				if (item instanceof Layer && !item._parent && this._remove(false, true)) {
					Base.splice(item._project.layers, [this], item._index + (above ? 1 : -1), 0);
	...
	var Raster = this.Raster = PlacedItem.extend({
		...
		, getPpi: function () {
			...
			return Size.create(72 / u.getLength(), 72 / v.getLength());
		}
		, getContext: function () {
		...
		, getSubImage: function (rect) {
			...
			canvas.getContext('2d').drawImage(this.getCanvas(), rect.x, rect.y, canvas.width, canvas.height, 0, 0, canvas.width, canvas.height);
			return canvas;
		}
		...
		, getAverageColor: function (object) {
			...
			if (!ctx) {
				ctx = Raster._sampleContext = CanvasProvider.getCanvas(new Size(sampleSize)).getContext('2d');
			}
			else {
		...
		, getPixel: function (point) {
			...
			for (var i = 0; i < 4; i++) channels[i] = pixels[i] / 255;
		...
		, getData: function (rect) {
			rect = Rectangle.read(arguments);
			if (rect.isEmpty()) rect = new Rectangle(this.getSize());
		...
		, drawSelected: function (ctx, matrix) {
			Item.drawSelectedBounds(new Rectangle(this._size).setCenter(0, 0), ctx, matrix);
		}
	});
	var PlacedSymbol = this.PlacedSymbol = PlacedItem.extend({
		...
		, drawSelected: function (ctx, matrix) {
			Item.drawSelectedBounds(this.symbol._definition.getBounds(), ctx, matrix);
		}
	});
	...
	var Segment = this.Segment = Base.extend({
		...
		, _changed: function (point) {
			...
			if (curve) {
				curve._changed();
				if (other = (curve[point == this._point || point == this._handleIn && curve._segment1 == this ? 'getPrevious' : 'getNext']())) {
					other._changed();
				}
			}
		...
		, _isSelected: function (point) {
			...
			return point == this._point ? !!(state & SelectionState.POINT) : point == this._handleIn ? !!(state & SelectionState.HANDLE_IN) : point == this._handleOut ? !!(state & SelectionState.HANDLE_OUT) : false;
		}
			...
			if (point == this._point) {
				if (selected) {
					...
					var previous = this.getPrevious()
						, next = this.getNext();
					selection[1] = previous && (previous._point.isSelected() || previous._handleOut.isSelected());
					selection[2] = next && (next._point.isSelected() || next._handleIn.isSelected());
				}
				...
				if (selection[index] != selected) {
					if (selected) selection[0] = false;
					selection[index] = selected;
					path._changed(Change.ATTRIBUTE);
				}
			}
			this._selectionState = (selection[0] ? SelectionState.POINT : 0) | (selection[1] ? SelectionState.HANDLE_IN : 0) | (selection[2] ? SelectionState.HANDLE_OUT : 0);
			if (path && state != this._selectionState) path._updateSelection(this, state, this._selectionState);
		}
		, isSelected: function () {
		...
		, getCurve: function () {
			if (this._path) {
				...
				if (!this._path._closed && index == this._path._segments.length - 1) index--;
		...
		, equals: function (segment) {
			return segment == this || segment && this._point.equals(segment._point) && this._handleIn.equals(segment._handleIn) && this._handleOut.equals(segment._handleOut);
		}
		, toString: function () {
	...
	var SegmentPoint = Point.extend({
		...
		, statics: {
			create: function (segment, key, pt) {
				...
				segment[key] = point;
				if (selected) point.setSelected(true);
				return point;
			}
		}
	});
	...
	var Curve = this.Curve = Base.extend({
		initialize: function (arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7) {
			...
			if (count == 0) {
				...
				this._segment1 = new Segment(p1, null, Point.create(arg2, arg3).subtract(p1));
				this._segment2 = new Segment(p2, Point.create(arg4, arg5).subtract(p2), null);
			}
		}
		, _changed: function () {
			delete this._length;
		...
		, getPoints: function () {
			var coords = this.getValues()
				, points = [];
			for (var i = 0; i < 8; i += 2) points.push(Point.create(coords[i], coords[i + 1]));
			return points;
		}
		, getLength: function () {
			...
			if (fullLength) this._length = length;
			return length;
		}
		...
		, getParameterAt: function (offset, start) {
			return Curve.getParameterAt(this.getValues(), offset, start !== undefined ? start : offset < 0 ? 1 : 0);
		}
		...
		, getCrossings: function (point, roots) {
			...
			for (var i = 0; i < num; i++) {
				...
				if (t >= 0 && t < 1 && Curve.evaluate(vals, t, 0).x > point.x) {
					if (t < Numerical.TOLERANCE && Curve.evaluate(this.getPrevious().getValues(), 1, 1).y * Curve.evaluate(vals, t, 1).y >= 0) continue;
					crossings++;
				}
			}
			return crossings;
		...
		, statics: {
			...
			, evaluate: function (v, t, type) {
				...
				if (type == 0 && (t == 0 || t == 1)) {
					...
					if (t < tMin && c1x == p1x && c1y == p1y) t = tMin;
					else if (t > 1 - tMin && c2x == p2x && c2y == p2y) t = 1 - tMin;
					var cx = 3 * (c1x - p1x)
						, bx = 3 * (c2x - c1x) - cx
						, ax = p2x - p1x - cx - bx
						, cy = 3 * (c1y - p1y)
			...
			, solveCubic: function (v, coord, val, roots) {
				var p1 = v[coord]
					...
					, b = 3 * (c2 - c1) - c
					, a = p2 - p1 - c - b;
				return Numerical.solveCubic(a, b, c, p1 - val, roots, Numerical.TOLERANCE);
			}
					...
					if (sx == -1 || (tx = txs[cx++]) >= 0 && tx <= 1) {
						for (var cy = 0; sy == -1 || cy < sy;) {
							if (sy == -1 || (ty = tys[cy++]) >= 0 && ty <= 1) {
								...
								if (Math.abs(tx - ty) < Numerical.TOLERANCE) return (tx + ty) * 0.5;
							}
						}
						if (sx == -1) break;
					}
		...
		return {
			...
			, getParameterAt: function (v, offset, start) {
				...
				function f(t) {
					...
					length += start < t ? Numerical.integrate(ds, start, t, count) : -Numerical.integrate(ds, t, start, count);
					start = t;
					return length - offset;
				}
				return Numerical.findRoot(f, ds, forward ? a + guess : b - guess, a, b, 16, Numerical.TOLERANCE);
			}
		};
	}, new function () {
		var maxDepth = 32
		...
		function toBezierForm(v, point) {
			...
			for (var i = 0; i <= n; i++) {
				...
				if (i < n) d[i] = v[i + 1].subtract(v[i]).multiply(n);
			}
			for (var row = 0; row < n; row++) {
				cd[row] = [];
				for (var column = 0; column <= n; column++) cd[row][column] = d[row].dot(c[column]);
			}
			for (var i = 0; i <= degree; i++) w[i] = new Point(i / degree, 0);
			for (k = 0; k <= degree; k++) {
		...
		function findRoots(w, depth) {
			...
			case 0:
				return [];
			case 1:
				if (depth >= maxDepth) return [0.5 * (w[0].x + w[5].x)];
				if (isFlatEnough(w)) {
			...
			var p = [[]]
				, left = []
				, right = [];
			for (var j = 0; j <= 5; j++) p[0][j] = new Point(w[j]);
			for (var i = 1; i <= 5; i++) {
				p[i] = [];
				for (var j = 0; j <= 5 - i; j++) p[i][j] = p[i - 1][j].add(p[i - 1][j + 1]).multiply(0.5);
			}
			for (var j = 0; j <= 5; j++) {
				left[j] = p[j][0];
				right[j] = p[5 - j][j];
		...
		function countCrossings(v) {
			...
			for (var i = 0, l = v.length; i < l; i++) {
				...
				if (prevSign != null && sign != prevSign) crossings++;
				prevSign = sign;
			}
			return crossings;
		}
		...
		function isFlatEnough(v) {
			...
			return Math.abs((maxAbove + maxBelow) / (2 * a * (a * a + b * b))) < epsilon;
		}
		return {
	...
	CurveLocation = Base.extend({
		...
		, getSegment: function () {
			if (!this._segment) {
				...
				if (parameter == 0) {
					...
					this._segment = curve.getLength(0, parameter) < curve.getLength(parameter, 1) ? curve._segment1 : curve._segment2;
				}
			}
		...
		, getCurveOffset: function () {
			...
			return parameter != null && this._curve && this._curve.getLength(0, parameter);
		}
		...
		, getTangent: function () {
			var parameter = this.getParameter();
			return parameter != null && this._curve && this._curve.getTangent(parameter);
		}
		, getNormal: function () {
			var parameter = this.getParameter();
			return parameter != null && this._curve && this._curve.getNormal(parameter);
		}
		...
		, toString: function () {
			...
			var parameter = this.getParameter();
			if (parameter != null) parts.push('parameter: ' + Base.formatNumber(parameter));
			if (this._distance != null) parts.push('distance: ' + Base.formatNumber(this._distance));
			return '{ ' + parts.join(', ') + ' }';
		}
	});
	...
	var Path = this.Path = PathItem.extend({
		initialize: function (segments) {
			...
			this.setSegments(!segments || !Array.isArray(segments) || typeof segments[0] !== 'object' ? arguments : segments);
		}
		, clone: function () {
		...
		, getCurves: function () {
			if (!this._curves) {
				...
				for (var i = 0; i < length; i++) this._curves[i] = Curve.create(this, segments[i], segments[i + 1] || segments[0]);
			}
			return this._curves;
		}
			...
			if (this._closed != (closed = !!closed)) {
				...
				if (this._curves) {
					var length = this._segments.length
						, i;
					if (!closed && length > 0) length--;
					this._curves.length = length;
					if (closed) this._curves[i = length - 1] = Curve.create(this, this._segments[i], this._segments[0]);
				}
				this._changed(Change.GEOMETRY);
			}
		}
		...
		, _apply: function (matrix) {
			...
			var style = this._style
				, fillColor = style._fillColor
				, strokeColor = style._strokeColor;
			if (fillColor && fillColor.transform) fillColor.transform(matrix);
			if (strokeColor && strokeColor.transform) strokeColor.transform(matrix);
			return true;
		}
		, _add: function (segs, index) {
			...
			for (var i = 0; i < amount; i++) {
				...
				if (fullySelected) segment._selectionState = SelectionState.POINT;
			...
			if (curves && --index >= 0) {
				curves.splice(index, 0, Curve.create(this, segments[index], segments[index + 1]));
		...
		, removeSegments: function (from, to) {
			...
			if (curves) {
				...
				var curve;
				if (curve = curves[from - 1]) curve._segment2 = segments[from];
				if (curve = curves[from]) curve._segment1 = segments[from];
				if (last && this._closed && (curve = curves[curves.length - 1])) curve._segment2 = segments[0];
			}
			this._changed(Change.GEOMETRY);
			return removed;
		}
		...
		, setFullySelected: function (selected) {
			var length = this._segments.length;
			this._selectedSegmentState = selected ? length * SelectionState.POINT : 0;
			for (var i = 0; i < length; i++) this._segments[i]._selectionState = selected ? SelectionState.POINT : 0;
		...
		, isClockwise: function () {
			...
			function edge(x, y) {
				if (xPre !== undefined) sum += (xPre - x) * (y + yPre);
				xPre = x;
				yPre = y;
			}
			for (var i = 0, l = this._segments.length; i < l; i++) {
		...
		, join: function (path) {
			if (path) {
				...
				if (last1._point.equals(last2._point)) path.reverse();
				...
				if (last1._point.equals(first2._point)) {
					...
					if (first1._point.equals(first2._point)) path.reverse();
					last2 = path.getLastSegment();
		...
		, getLocation: function (point) {
			...
			for (var i = 0, l = curves.length; i < l; i++) {
				...
				if (t != null) return new CurveLocation(curve, t);
			}
			return null;
		}
		, getLocationAt: function (offset, isParameter) {
			...
			for (var i = 0, l = curves.length; i < l; i++) {
				...
				if (length >= offset) {
					return new CurveLocation(curve, curve.getParameterAt(offset - start));
				}
			}
		...
		, contains: function (point) {
			point = Point.read(arguments);
			if (!this._closed || !this.getRoughBounds()._containsPoint(point)) return false;
			var curves = this.getCurves()
				, crossings = 0
				, roots = [];
			for (var i = 0, l = curves.length; i < l; i++) crossings += curves[i].getCrossings(point, roots);
			return (crossings & 1) == 1;
		}
		, _hitTest: function (point, options) {
			var style = this._style
				, tolerance = options.tolerance || 0
				, radius = (options.stroke && style._strokeColor ? style._strokeWidth / 2 : 0) + tolerance
				, loc
				, res;
			var coords = []
				, that = this;
			...
			function checkPoint(seg, pt, name) {
				if (point.getDistance(pt) < tolerance) return new HitResult(name, that, {
					segment: seg
					, point: pt
				});
			}
			...
			function checkSegment(seg, ends) {
				...
				return (ends || options.segments) && checkPoint(seg, point, 'segment') || (!ends && options.handles) && (checkPoint(seg, point.add(seg._handleIn), 'handle-in') || checkPoint(seg, point.add(seg._handleOut), 'handle-out'));
			}
			if (options.ends && !options.segments && !this._closed) {
				if (res = checkSegment(this.getFirstSegment(), true) || checkSegment(this.getLastSegment(), true)) return res;
			}
			else if (options.segments || options.handles) {
				for (var i = 0, l = this._segments.length; i < l; i++) {
					if (res = checkSegment(this._segments[i])) return res;
				}
			}
			if (options.stroke && radius > 0) loc = this.getNearestLocation(point);
			if (!(loc && loc._distance <= radius) && options.fill && style._fillColor && this.contains(point)) return new HitResult('fill', this);
			if (!loc && options.stroke && radius > 0) loc = this.getNearestLocation(point);
			if (loc && loc._distance <= radius) return options.stroke ? new HitResult('stroke', this, {
				location: loc
		...
		function drawHandles(ctx, segments, matrix) {
			...
			for (var i = 0, l = segments.length; i < l; i++) {
				...
				if (selected || (state & SelectionState.HANDLE_IN)) drawHandle(2);
				if (selected || (state & SelectionState.HANDLE_OUT)) drawHandle(4);
				ctx.save();
				ctx.beginPath();
		...
		function drawSegments(ctx, path, matrix) {
			...
			for (var i = 0; i < length; i++) drawSegment(i);
			if (path._closed && length > 1) drawSegment(0);
		}
		return {
			draw: function (ctx, param) {
				if (!param.compound) ctx.beginPath();
				var style = this._style
					, fillColor = style._fillColor
					, strokeColor = style._strokeColor
					, dashArray = style._dashArray
					, hasDash = strokeColor && dashArray && dashArray.length;
				if (param.compound || this._clipMask || fillColor || strokeColor && !hasDash) {
		...
		return {
			smooth: function () {
				...
				for (var i = overlap; i <= n - overlap; i++) {
					...
					if (i < n) {
						segment.setHandleOut(Point.create(x[i], y[i]).subtract(segment._point));
						if (i < n - 1) handleIn = Point.create(2 * knots[i + 1]._x - x[i + 1], 2 * knots[i + 1]._y - y[i + 1]);
						else handleIn = Point.create(
							(knots[n]._x + x[n - 1]) / 2, (knots[n]._y + y[n - 1]) / 2);
					}
				}
		...
		return {
			...
			, arcTo: function (to, clockwise) {
				...
				var l1 = new Line(from.add(through).divide(2), through.subtract(from).rotate(90))
					, l2 = new Line(through.add(to).divide(2), to.subtract(through).rotate(90))
					, center = l1.intersect(l2)
					, line = new Line(from, to, true)
					, throughSide = line.getSide(through);
				if (!center) {
					if (!throughSide) return this.lineTo(to);
					throw new Error("Cannot put an arc through the given points: " + [from, through, to]);
				}
				...
				for (var i = 0; i <= count; i++) {
					...
					if (i == 0) {
						...
						segments.push(new Segment(pt, vector.rotate(-90).multiply(z), out));
					}
		...
		function getBounds(matrix, strokePadding) {
			...
			function processSegment(segment) {
				...
				for (var i = 0; i < 2; i++) {
					...
					function add(value, t) {
						...
						if (value == null) {
							var u = 1 - t;
							value = u * u * u * v0 + 3 * u * u * t * v1 + 3 * u * t * t * v2 + t * t * t * v3;
							padding = strokePadding ? strokePadding[i] : 0;
						}
						var left = value - padding
							, right = value + padding;
						if (left < min[i]) min[i] = left;
						if (right > max[i]) max[i] = right;
					}
					add(v3, null);
					var a = 3 * (v1 - v2) - v0 + v3
						, b = 2 * (v0 + v2) - 4 * v1
					...
					var sqrt = Math.sqrt(q)
						...
						, t1 = (b - sqrt) * f
						, t2 = (b + sqrt) * f;
					if (tMin < t1 && t1 < tMax) add(null, t1);
					if (tMin < t2 && t2 < tMax) add(null, t2);
				}
				var tmp = prevCoords;
				prevCoords = coords;
				coords = tmp;
			...
			if (this._closed) processSegment(first);
		...
		function getStrokeBounds(matrix) {
			var style = this._style;
			if (!style._strokeColor || !style._strokeWidth) return getBounds.call(this, matrix);
			...
			function addJoin(segment, join) {
				if (join === 'round' || !segment._handleIn.isZero() && !segment._handleOut.isZero()) {
					...
					var curve2 = segment.getCurve()
						...
						, line1 = new Line(point.subtract(normal1), Point.create(-normal1.y, normal1.x))
						, line2 = new Line(point.subtract(normal2), Point.create(-normal2.y, normal2.x))
			...
			function addCap(segment, cap, t) {
				...
				case 'square':
					var curve = segment.getCurve()
						...
						, normal = curve.getNormal(t).normalize(radius);
					if (cap === 'square') point = point.add(normal.rotate(t == 0 ? -90 : 90));
					add(point.add(normal));
					add(point.subtract(normal));
					break;
				}
		...
		function getRoughBounds(matrix) {
			var style = this._style
				, width = style._strokeWidth;
			return getHandleBounds.call(this, matrix, width, style._strokeJoin == 'miter' ? width * style._miterLimit : width);
		}
		var get = {
			bounds: getBounds
		...
		, new Segment([0.5, 1], [kappa, 0], [-kappa, 0])
			...
			return {
				Line: function () {
					...
					return new Path(Segment.read(arguments, 0, step), Segment.read(arguments, step, step));
				}
				...
				, Oval: function (rect) {
					...
					for (var i = 0; i < 4; i++) {
						...
						segments[i] = new Segment(segment._point.multiply(size).add(point), segment._handleIn.multiply(size), segment._handleOut.multiply(size));
					}
					path._add(segments);
					path._closed = true;
					return path;
				...
				, Circle: function (center, radius) {
					...
					return Path.Oval(new Rectangle(center.subtract(radius), Size.create(radius * 2, radius * 2)));
				}
	...
	var CompoundPath = this.CompoundPath = PathItem.extend({
		initialize: function (paths) {
			...
			this._children = [];
			this._namedChildren = {};
			var items = !paths || !Array.isArray(paths) || typeof paths[0] !== 'object' ? arguments : paths;
			this.addChildren(items);
		}
		, insertChild: function (index, item) {
		...
		, draw: function (ctx, param) {
			...
			ctx.beginPath();
			param.compound = true;
			for (var i = 0, l = children.length; i < l; i++) Item.draw(children[i], ctx, param);
			firstChild._setStyles(ctx);
			if (style._fillColor) ctx.fill();
			if (style._strokeColor) ctx.stroke();
			param.compound = false;
		}
	}, new function () {
		function getCurrentPath(that) {
			if (!that._children.length) throw new Error('Use a moveTo() command first');
		...
		var fields = {
			...
			, moveBy: function (point) {
				this.moveTo(getCurrentPath(this).getLastSegment()._point.add(Point.read(arguments)));
			}
			, closePath: function () {
	...
	var PathFitter = Base.extend({
		...
		, findRoot: function (curve, point, u) {
			...
			if (Math.abs(df) < Numerical.TOLERANCE) return u;
	...
	var PointText = this.PointText = TextItem.extend({
		...
		, draw: function (ctx) {
			...
			for (var i = 0, l = lines.length; i < l; i++) {
				var line = lines[i];
				if (style._fillColor) ctx.fillText(line, 0, 0);
				if (style._strokeColor) ctx.strokeText(line, 0, 0);
		...
		return {
			_getBounds: function (type, matrix) {
				if (!context) context = CanvasProvider.getCanvas(Size.create(1, 1)).getContext('2d');
				var justification = this.getJustification()
					, x = 0;
				context.font = this._style.getFontStyle();
				var width = 0;
				for (var i = 0, l = this._lines.length; i < l; i++) width = Math.max(width, context.measureText(this._lines[i]).width);
				if (justification !== 'left') x -= width / (justification === 'center' ? 2 : 1);
				var leading = this.getLeading()
					, count = this._lines.length
					, bounds = Rectangle.create(x, count ? leading / 4 + (count - 1) * leading : 0, width, -count * leading);
	...
	var Style = Item.extend({
		initialize: function (style) {
			...
			return Base.each(this._defaults, function (value, key) {
				...
				this[key] = value && clone && value.clone ? value.clone() : value;
			}, this);
		}
		, statics: {
			...
			, extend: function (src) {
				...
				Base.each(src._defaults, function (value, key) {
					...
					src[set] = function (value) {
						...
						if (children) {
							for (var i = 0, l = children.length; i < l; i++) children[i][styleKey][set](value);
						}
						else {
							var old = this['_' + key];
							if (old != value && !(old && old.equals && old.equals(value))) {
								this['_' + key] = value;
								if (isColor) {
									if (old) old._removeOwner(this._item);
									if (value) value._addOwner(this._item);
								}
								if (this._item) this._item._changed(flags[key] || Change.STYLE);
							}
						}
						return this;
					};
					src[get] = function () {
						var children = this._item && this._item._children
							, style;
						if (!children) return this['_' + key];
						for (var i = 0, l = children.length; i < l; i++) {
							var childStyle = children[i][styleKey][get]();
							if (!style) {
								style = childStyle;
							}
							else if (style != childStyle && !(style && style.equals && style.equals(childStyle))) {
								return undefined;
							}
						}
						return style;
	...
	var Color = this.Color = Base.extend(new function () {
		...
		function hexToRgbColor(string) {
			...
			if (hex.length >= 4) {
				...
				for (var i = 0; i < 3; i++) {
					var channel = hex[i + 1];
					rgb[i] = parseInt(channel.length == 1 ? channel + channel : channel, 16) / 255;
				}
		...
		var converters = {
			...
			, 'hsl-rgb': function (color) {
				var s = color._saturation
					...
					, l = color._lightness
					, t1, t2, c;
				if (s == 0) return new RgbColor(l, l, l, color._alpha);
			...
			, 'rgb-gray': function (color) {
				return new GrayColor(1 - (color._red * 0.2989 + color._green * 0.587 + color._blue * 0.114), color._alpha);
			}
		...
		var fields = {
			...
			, initialize: function (arg) {
				...
				if (typeof arg === 'object' && !isArray) {
					...
					var rgbColor = arg.match(/^#[0-9a-f]{3,6}$/i) ? hexToRgbColor(arg) : nameToRgbColor(arg);
					return type ? rgbColor.convert(type) : rgbColor;
				}
				else {
					var components = isArray ? arg : Array.prototype.slice.call(arguments);
					if (!type) {
						if (components.length >= 3) return new RgbColor(components);
						return new GrayColor(components);
					}
					else {
						Base.each(this._components, function (name, i) {
							var value = components[i];
							this['_' + name] = value !== undefined ? value : null;
						}, this);
					}
				}
			}
			...
			, convert: function (type) {
				var converter;
				return this._colorType == type ? this.clone() : (converter = converters[this._colorType + '-' + type]) ? converter(this) : converters['rgb-' + type](converters[this._colorType + '-rgb'](this));
			}
			, statics: {
				extend: function (src) {
					if (src._colorType) {
						...
						Base.each(comps, function (name) {
							...
							this['set' + part] = function (value) {
								this[name] = isHue ? ((value % 360) + 360) % 360 : Math.min(Math.max(value, 0), 1);
								this._changed();
								return this;
							};
						}, src);
		...
		_changed: function () {
			...
			for (var i = 0, l = this._owners && this._owners.length; i < l; i++) this._owners[i]._changed(Change.STYLE);
		}
		...
		, getComponents: function () {
			...
			for (var i = 0; i < length; i++) comps[i] = this['_' + this._components[i]];
			return comps;
		}
		, getAlpha: function () {
		...
		, equals: function (color) {
			if (color && color._colorType === this._colorType) {
				for (var i = 0, l = this._components.length; i < l; i++) {
					...
					if (this[component] !== color[component]) return false;
				}
				return true;
			}
			return false;
	...
	var GradientColor = this.GradientColor = Color.extend({
		initialize: function (gradient, origin, destination, hilite) {
			...
			if (hilite) this.setHilite(hilite);
		}
		, clone: function () {
		...
		, setHilite: function (hilite) {
			...
			if (vector.getLength() > this._radius) {
				this._hilite = this._origin.add(vector.normalize(this._radius - 0.1));
			}
			else {
				this._hilite = hilite;
			}
		...
		, getCanvasStyle: function (ctx) {
			...
			if (this.gradient.type === 'linear') {
				gradient = ctx.createLinearGradient(this._origin.x, this._origin.y, this._destination.x, this._destination.y);
			}
			else {
				var origin = this._hilite || this._origin;
				gradient = ctx.createRadialGradient(origin.x, origin.y, 0, this._origin.x, this._origin.y, this._radius);
			}
		...
		, equals: function (color) {
			return color == this || color && color._colorType === this._colorType && this.gradient.equals(color.gradient) && this._origin.equals(color._origin) && this._destination.equals(color._destination);
		}
		, transform: function (matrix) {
			matrix._transformPoint(this._origin, this._origin, true);
			matrix._transformPoint(this._destination, this._destination, true);
			if (this._hilite) matrix._transformPoint(this._hilite, this._hilite, true);
			this._radius = this._destination.getDistance(this._origin);
		}
	});
	var Gradient = this.Gradient = Base.extend({
		...
		, equals: function (gradient) {
			if (gradient.type != this.type) return false;
	...
	var GradientStop = this.GradientStop = Base.extend({
		...
		, setColor: function (color) {
			if (this._color) this._color._removeOwner(this);
			this._color = Color.read(arguments);
			this._color._addOwner(this);
			this._changed();
		}
		, equals: function (stop) {
			return stop == this || stop instanceof GradientStop && this._color.equals(stop._color) && this._rampPoint == stop._rampPoint;
		}
	});
	var DomElement = {
		...
		, isVisible: function (el) {
			return !this.isInvisible(el) && this.getViewportBounds(el).intersects(this.getBounds(el, true));
		}
		...
		, getViewportBounds: function (el) {
			var doc = el.ownerDocument
				...
				, body = doc.getElementsByTagName(doc.compatMode === 'CSS1Compat' ? 'html' : 'body')[0];
			return Rectangle.create(0, 0, view.innerWidth || body.clientWidth, view.innerHeight || body.clientHeight);
		}
		, getComputedStyle: function (el, name) {
			if (el.currentStyle) return el.currentStyle[Base.camelize(name)];
			var style = this.getViewport(el.ownerDocument).getComputedStyle(el, null);
	...
	var DomEvent = {
		...
		, getPoint: function (event) {
			...
			return Point.create(pos.pageX || pos.clientX + document.documentElement.scrollLeft, pos.pageY || pos.clientY + document.documentElement.scrollTop);
		}
	...
	DomEvent.requestAnimationFrame = new function () {
		var part = 'equestAnimationFrame'
			, request = window['r' + part] || window['webkitR' + part] || window['mozR' + part] || window['oR' + part] || window['msR' + part];
		if (request) {
			request(function (time) {
				if (time == undefined) request = null;
			});
		}
		var callbacks = []
			, focused = true
		...
		return function (callback, element) {
			if (request) return request(callback, element);
			...
			timer = window.setInterval(function () {
				for (var i = callbacks.length - 1; i >= 0; i--) {
					var entry = callbacks[i]
						, func = entry[0]
						, el = entry[1];
					if (!el || (PaperScript.getAttribute(el, 'keepalive') == 'true' || focused) && DomElement.isVisible(el)) {
	...
	var View = this.View = Base.extend(Callback, {
			...
			onFrame: {
				install: function () {
					...
					this._onFrameCallback = function (param, dontRequest) {
						requested = false;
						if (!that._onFrameCallback) return;
						paper = that._scope;
						if (!dontRequest) {
							requested = true;
							DomEvent.requestAnimationFrame(that._onFrameCallback, that._element);
						}
						...
						before = now;
						if (that._stats) that._stats.update();
						that.draw(true);
					};
					if (!requested) this._onFrameCallback();
				}
				, uninstall: function () {
		...
		, initialize: function (element) {
			...
			if (PaperScript.hasAttribute(element, 'resize')) {
				var offset = DomElement.getOffset(element, true)
					, that = this;
				size = DomElement.getViewportBounds(element).getSize().subtract(offset);
				element.width = size.width;
				element.height = size.height;
				DomEvent.add(window, {
					resize: function (event) {
						if (!DomElement.isInvisible(element)) offset = DomElement.getOffset(element, true);
						that.setViewSize(DomElement.getViewportBounds(element).getSize().subtract(offset));
					}
				});
			}
			else {
				size = DomElement.isInvisible(element) ? Size.create(parseInt(element.getAttribute('width')), parseInt(element.getAttribute('height'))) : DomElement.getSize(element);
			}
			...
			this._viewSize = LinkedSize.create(this, 'setViewSize', size.width, size.height);
		...
		, remove: function () {
			...
			if (this._project.view == this) this._project.view = null;
		...
		function mousedown(event) {
			...
			dragging = true;
			if (view._onMouseDown) view._onMouseDown(event, curPoint);
			if (tool = view._scope.tool) tool._onHandleEvent('mousedown', curPoint, event);
			view.draw(true);
		}
		...
		function mousemove(event) {
			...
			if (!(view = view || View._focused)) return;
			var point = event && viewToProject(view, event);
			if (view._onMouseMove) view._onMouseMove(event, point);
			if (tool = view._scope.tool) {
				var onlyMove = !!(!tool.onMouseDrag && tool.onMouseMove);
				if (dragging && !onlyMove) {
					if ((curPoint = point || curPoint) && tool._onHandleEvent('mousedrag', curPoint, event)) DomEvent.stop(event);
				}
				else if ((!dragging || onlyMove) && tool._onHandleEvent('mousemove', point, event)) {
					DomEvent.stop(event);
				}
			}
			view.draw(true);
		...
		function mouseup(event) {
			...
			curPoint = null;
			dragging = false;
			if (view._onMouseUp) view._onMouseUp(event, point);
			if (tool && tool._onHandleEvent('mouseup', point, event)) DomEvent.stop(event);
			view.draw(true);
		}
	...
	var CanvasView = View.extend({
		initialize: function (canvas) {
			if (!(canvas instanceof HTMLCanvasElement)) {
				...
				if (size.isZero()) size = Size.create(1024, 768);
		...
		, draw: function (checkRedraw) {
			if (checkRedraw && !this._redrawNeeded) return false;
		...
		function callEvent(type, event, point, target, lastPoint, bubble) {
			...
			while (item) {
				if (item.responds(type)) {
					if (!mouseEvent) mouseEvent = new MouseEvent(type, event, point, target, lastPoint ? point.subtract(lastPoint) : null);
					called = item.fire(type, mouseEvent) || called;
					if (called && (!bubble || mouseEvent._stopped)) break;
				}
				item = item.getParent();
			}
			return called;
		...
		function handleEvent(view, type, event, point, lastPoint) {
			if (view._eventCounters[type]) {
				var hit = view._project.hitTest(point, hitOptions)
					, item = hit && hit.item;
				if (item) {
					if (type == 'mousemove' && item != overItem) lastPoint = point;
					if (type != 'mousemove' || !hasDrag) callEvent(type, event, point, item, lastPoint);
					return item;
				}
			}
		}
		return {
			...
			, _onMouseUp: function (event, point) {
				...
				if (hasDrag) {
					if (lastPoint && !lastPoint.equals(point)) callEvent('mousedrag', event, point, downItem, lastPoint);
					if (item != downItem) {
						overPoint = point;
				...
				if (item == downItem) {
					clickTime = Date.now();
					callEvent(doubleClick ? 'doubleclick' : 'click', event, downPoint, overItem);
					doubleClick = false;
				}
				downItem = null;
				hasDrag = false;
			...
			, _onMouseMove: function (event, point) {
				if (downItem) callEvent('mousedrag', event, point, downItem, lastPoint);
		...
		DomEvent.add(document, {
			keydown: function (event) {
				...
				var key = keys[code]
					, name;
				if (key) {
					if ((name = Base.camelize(key)) in modifiers) modifiers[name] = true;
					charCodeMap[code] = 0;
			...
			, keyup: function (event) {
				var code = event.which || event.keyCode
					, key = keys[code]
					, name;
				if (key && (name = Base.camelize(key)) in modifiers) modifiers[name] = false;
			...
			if (!this._item) {
				...
				if (result) {
					var item = result.item
						, parent = item._parent;
					while ((parent instanceof Group && !(parent instanceof Layer)) || parent instanceof CompoundPath) {
						item = parent;
						parent = parent._parent;
					}
					this._item = item;
	...
	var Tool = this.Tool = PaperScopeItem.extend(Callback, {
		...
		, _updateEvent: function (type, pt, minDistance, maxDistance, start, needsChange, matchMaxDistance) {
			if (!start) {
				...
				if (needsChange && pt.equals(this._point)) return false;
			}
		...
		, _onHandleEvent: function (type, pt, event) {
			...
			if (sets) {
				...
				if (set) {
					for (var id in set) {
						...
						for (var key in sets) {
							var other = sets[key];
							if (other && other != set && other[item._id]) delete other[item._id];
						}
						item.remove();
					}
					sets[type] = null;
			...
			case 'mousedrag':
				...
				while (this._updateEvent(type, pt, this.minDistance, this.maxDistance, false, needsChange, matchMaxDistance)) {
					if (this.responds(type)) called = this.fire(type, new ToolEvent(this, type, event));
					needsChange = true;
				...
				break;
			case 'mouseup':
				if ((this._point.x != pt.x || this._point.y != pt.y) && this._updateEvent('mousedrag', pt, this.minDistance, this.maxDistance, false, false, false)) {
					if (this.responds('mousedrag')) called = this.fire('mousedrag', new ToolEvent(this, type, event));
				}
				this._updateEvent(type, pt, null, this.maxDistance, false, false, false);
				if (this.responds(type)) called = this.fire(type, new ToolEvent(this, type, event));
				this._updateEvent(type, pt, null, null, true, false, false);
				this._firstMove = true;
				break;
			case 'mousemove':
				while (this._updateEvent(type, pt, this.minDistance, this.maxDistance, this._firstMove, true, false)) {
					if (this.responds(type)) called = this.fire(type, new ToolEvent(this, type, event));
		...
		, getCanvas: function (size) {
			if (this.canvases.length) {
				...
				if ((canvas.width != size.width) || (canvas.height != size.height)) {
					...
					canvas.getContext('2d').clearRect(0, 0, size.width + 1, size.height + 1);
				}
				return canvas;
			}
			else {
	...
	var Numerical = new function () {
		...
		return {
			...
			, solveQuadratic: function (a, b, c, roots, tolerance) {
				...
				q = (b + q) * -0.5;
				var n = 0;
				if (abs(q) >= tolerance) roots[n++] = c / q;
				if (abs(a) >= tolerance) roots[n++] = q / a;
				return n;
			}
			, solveCubic: function (a, b, c, d, roots, tolerance) {
				if (abs(a) < tolerance) return Numerical.solveQuadratic(b, c, d, roots, tolerance);
				b /= a;
				c /= a;
				d /= a;
				var Q = (b * b - 3 * c) / 9
	...
	var PaperScript = this.PaperScript = new function () {
		...
		function compile(code) {
			...
			ast = walker.with_walkers({
				'binary': function (operator, left, right) {
					...
					return handleOperator(operator, left = walk(left), right = walk(right))
						// Always return something since we're walking left and
						|| [this[0], operator, left, right];
				}
				, 'assign': function (operator, left, right) {
					var res = handleOperator(operator, left = walk(left), right = walk(right));
					if (res) return [this[0], true, left, res];
		...
		function load() {
			...
			for (var i = 0, l = scripts.length; i < l; i++) {
				...
				if (/^text\/(?:x-|)paperscript$/.test(script.type) && !script.getAttribute('data-paper-loaded')) {
